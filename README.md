...
Apps generadas (Angular 21 zoneless + SCSS, NestJS 12 + Prisma 7 con adaptador pg, Vite 8 + Sass). Modelo de datos en `database/` (árbol de unidades, usuarios y contratos). API con CRUD de usuarios y unidades y métricas (ver `services/api/README.md`). Dashboard del administrador en `client/` (ver `client/README.md`). Las unidades se eliminan de forma lógica (`deleted_at`) y se pueden restaurar. Pendiente: autenticación y gestión de contratos. Requiere `npm install` en `client/`, `services/api/` y `landing/`.
