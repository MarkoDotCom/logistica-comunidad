...
- **Contratos**: la tabla `app-unit-contracts` de las páginas de detalle ofrece "+ Nuevo contrato" y "Modificar" por fila; ambos abren `app-contract-wizard` (Presentación → Persona → Datos → Confirmar). La persona se busca entre los usuarios (`UsersApi.list(search)`), el tipo se limita a los que admite la unidad (`allowedContractTypes`), y al cerrar la tabla emite `changed` para que la página recargue el detalle.
