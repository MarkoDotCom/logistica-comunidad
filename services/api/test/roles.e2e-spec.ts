...
const MARCELA = '50000000-0000-4000-8000-000000000001';
...
describe('Roles (e2e)', () => {
  ...
  it('lists the permission catalog and the roles with counts', async () => {
    const perms = await request(app.getHttpServer()).get('/permissions').expect(200);
  ...
  it('creates, reads, updates and soft-deletes a role', async () => {
    const created = await request(app.getHttpServer()).post('/roles').send({ name: `Contador ${suffix}`, description: 'Ve cuentas', permissions: ['units.read', 'contracts.read'] }).expect(201);
    ...
    await request(app.getHttpServer()).post('/roles').send({ name: `contador ${suffix}`, permissions: [] }).expect(409); // citext
    await request(app.getHttpServer()).post('/roles').send({ name: 'X', permissions: ['nope.read'] }).expect(400);
    await request(app.getHttpServer()).post('/roles').send({ name: 'Mal', permissions: ['sin-punto'] }).expect(400);

    const updated = await request(app.getHttpServer()).patch(`/roles/${created.body.data.id}`).send({ permissions: ['users.read'], description: null }).expect(200);
    expect(updated.body.data).toMatchObject({ permissions: ['users.read'], description: null, permissionCount: 1 });

    await request(app.getHttpServer()).delete(`/roles/${created.body.data.id}`).expect(204);
    await request(app.getHttpServer()).get(`/roles/${created.body.data.id}`).expect(404);
  ...
  it('protects system roles and assigns users from both sides', async () => {
    await request(app.getHttpServer()).patch(`/roles/${ADMIN_ROLE}`).send({ name: 'Root' }).expect(400);
    await request(app.getHttpServer()).delete(`/roles/${ADMIN_ROLE}`).expect(400);

    const role = await request(app.getHttpServer()).post('/roles').send({ name: `Comité ${suffix}`, permissions: ['units.read'] }).expect(201);
    ids.push(role.body.data.id);

    // Desde el rol: agregar a Bruno (idempotente) y quitarlo
    const added = await request(app.getHttpServer()).put(`/roles/${role.body.data.id}/users/${BRUNO}`).expect(200);
    expect(added.body.data.users).toEqual([expect.objectContaining({ fullName: 'Bruno Díaz' })]);
    await request(app.getHttpServer()).put(`/roles/${role.body.data.id}/users/${BRUNO}`).expect(200);
    const bruno = await request(app.getHttpServer()).get(`/users/${BRUNO}`).expect(200);
    expect(bruno.body.data.roles).toEqual(expect.arrayContaining([expect.objectContaining({ name: `Comité ${suffix}` })]));
    const removed = await request(app.getHttpServer()).delete(`/roles/${role.body.data.id}/users/${BRUNO}`).expect(200);
    expect(removed.body.data.users).toEqual([]);
    await request(app.getHttpServer()).put(`/roles/${role.body.data.id}/users/00000000-0000-4000-8000-000000000000`).expect(400);

    // Desde el usuario: roleIds reemplaza el conjunto; al final se restaura el de Marcela
    const before = (await request(app.getHttpServer()).get(`/users/${MARCELA}`)).body.data.roles.map((r: { id: string }) => r.id);
    const patched = await request(app.getHttpServer()).patch(`/users/${MARCELA}`).send({ roleIds: [role.body.data.id] }).expect(200);
