...
describe('Usuarios (e2e)', () => {
  ...
  it('lists users and filters by name or email', async () => {
    const all = await request(app.getHttpServer()).get('/users').set(session.auth).expect(200);
    expect(all.body.data.length).toBeGreaterThanOrEqual(5);

    const res = await request(app.getHttpServer()).get('/users?search=rojas').set(session.auth).expect(200);
  ...
  it('reads a user with their contracts and units', async () => {
    const res = await request(app.getHttpServer()).get(`/users/${ANA}`).set(session.auth).expect(200);
    ...
    await request(app.getHttpServer()).get('/users/00000000-0000-4000-8000-000000000000').set(session.auth).expect(404);
  });

  it('creates, updates and validates a user', async () => {
    const email = `e2e.${suffix}@example.com`;
    const created = await request(app.getHttpServer()).post('/users').set(session.auth).send({ email, fullName: 'Eva Soto' }).expect(201);
    ...
    const updated = await request(app.getHttpServer())
      .patch(`/users/${created.body.data.id}`).set(session.auth)
    ...
    await request(app.getHttpServer()).post('/users').set(session.auth).send({ email: 'no-es-email', fullName: 'X' }).expect(400);
    await request(app.getHttpServer()).post('/users').set(session.auth).send({ email: 'ANA.ROJAS@example.com', fullName: 'Ana' }).expect(409); // citext
    await request(app.getHttpServer()).patch(`/users/${created.body.data.id}`).set(session.auth).send({ email: 'ana.rojas@example.com' }).expect(409);
  });
});
